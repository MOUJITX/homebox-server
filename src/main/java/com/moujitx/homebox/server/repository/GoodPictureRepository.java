...
public interface GoodPictureRepository extends JpaRepository<GoodPicture, Long> {
    ...
    @Query("SELECT p.good.id AS goodId, p.file.storedFilename AS storedFilename FROM GoodPicture p WHERE p.id IN (SELECT MIN(p2.id) FROM GoodPicture p2 WHERE p2.good.id IN :goodIds GROUP BY p2.good.id)")
