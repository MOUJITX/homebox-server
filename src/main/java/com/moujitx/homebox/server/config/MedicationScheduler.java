...
public class MedicationScheduler {
    ...
    private static ThreadPoolTaskScheduler createScheduler() {
        ...
        scheduler.setThreadNamePrefix("medication-");
    ...
    public void scheduleFromConfig() {
        ...
        String cron = systemConfigService.get("notification.medication-crontab");
        if (cron.isEmpty()) {
            cron = "0 0 7-20 * * ?";
        }
        ...
        try {
            ...
            log.info("Medication check scheduled with cron: {}", cron);
        } catch (Exception e) {
            log.error("Failed to schedule medication check with cron: {}", cron, e);
        }
    }

    static String normalizeCron(String cron) {
        if (cron == null || cron.isEmpty()) return "0 0 7-20 * * ?";
    ...
    public void runCheck() {
        try {
            medicationService.checkAndNotify();
        } catch (Exception e) {
            log.error("Medication check failed", e);
        }
    }
}
