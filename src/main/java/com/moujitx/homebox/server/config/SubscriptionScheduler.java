...
public class SubscriptionScheduler {
    ...
    private static ThreadPoolTaskScheduler createScheduler() {
        ...
        scheduler.setThreadNamePrefix("subscription-");
    ...
    public void scheduleFromConfig() {
        ...
        String cron = systemConfigService.get("notification.subscription-crontab");
        if (cron.isEmpty()) {
            cron = "0 0 8 * * ?";
        }
        ...
        try {
            ...
            log.info("Subscription check scheduled with cron: {}", cron);
        } catch (Exception e) {
            log.error("Failed to schedule subscription check with cron: {}", cron, e);
        }
    }
    ...
    public void runCheck() {
        try {
            subscriptionService.checkAndNotify();
        } catch (Exception e) {
